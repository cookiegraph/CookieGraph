...
def log_prediction_probability(clf, df_feature_test, cols, test_mani, y_pred, result_dir, tag):
	...
	with open(fname, "w") as f:
		...
		for i in range(0, len(y_pred_prob)):
			...
			preds = ' |$| '.join(preds)
			f.write("%s |$| %s |$| %s |$| %s |$| %s\n" 
				%(truth_labels[i], pred_labels[i], preds, truth_names[i], truth_vids[i]))
...
def classify(train, test, result_dir, tag, sample, log_pred_probability):
	...
	fields_to_remove = ['visit_id', 'name', 'label', 'single']
	...
	if sample:
		...
		df_feature_train, train_labels = oversample.fit_resample(df_feature_train, train_labels)
		undersample = RandomUnderSampler(sampling_strategy=0.5)
		df_feature_train, train_labels = undersample.fit_resample(df_feature_train, train_labels)

		fname = os.path.join(result_dir, "composition")
		with open(fname, 'a') as f:
			counts = collections.Counter(train_labels)
			f.write("\nAfter sampling, new composition: " + str(counts['Positive']) + " " + get_perc(counts['Positive'], len(train_labels)) + "\n")
	...
	with open(fname, "a") as f:
		...
		f.write("Precision score: binary " + str(round(prec_binary*100, 3)) + "%" + "\n")
		f.write("Recall score: binary " + str(round(rec_binary*100, 3)) + "%" +  "\n")
		f.write("Precision score: micro " + str(round(prec_micro*100, 3)) + "%" + "\n")
		f.write("Recall score: micro " + str(round(rec_micro*100, 3)) + "%" +  "\n")
		f.write("Precision score: macro " + str(round(prec_macro*100, 3)) + "%" + "\n")
		f.write("Recall score: macro " + str(round(rec_macro*100, 3)) + "%" +  "\n")

	print("Accuracy Score:", acc)

	if log_pred_probability:
		log_prediction_probability(clf, df_feature_test, cols, test_mani, y_pred, result_dir, tag)
...
def classify_validation(df_train, df_validation, result_dir, sample=False, log_pred_probability=False):

	i = 0
	result = classify(df_train, df_validation, result_dir, i, sample, log_pred_probability)
	results = [result]

	return results

def classify_crossval(df_labelled, result_dir, sample=False, log_pred_probability=False):
	...
	for i in range(0, num_iter):
		...
		result = classify(df_train, df_test, result_dir, i, sample, log_pred_probability)
...
def pipeline(df_features, df_labels, result_dir):
	...
	df = df_features.merge(df_labels, on=['name'])
	...
	results = classify_crossval(df_labelled, result_dir, sample=False, log_pred_probability=True)
	...
	#esults = classify_validation(df_crossval, df_validation, valid_result_dir, sample=False, log_pred_probability=True)
...
if __name__ == "__main__":
	...
	FEATURE_FOLDER = "../features_final_run"
	LABEL_FOLDER = "../labels_run5"
	RESULT_DIR = "results/10k_run14"
	...
	#df_labels = df_labels[(df_labels['setter'].notnull()) & (df_labels['domain'].notnull())]
	#df_labels = df_labels[['visit_id', 'name', 'label']].drop_duplicates()
	df_labels = pd.read_pickle("../newlabels_run7_processed.pkl")
	df_multi = pd.read_pickle("../single_with_ls.pkl")
	df_labels = df_labels.merge(df_multi[['visit_id', 'name', 'single']], on=['visit_id', 'name'], how='inner')
	df_labels = df_labels[df_labels['single'] == "Multi"]
