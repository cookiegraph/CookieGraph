...
def get_storage_features(df_graph, node):
  ...
  try:
    cookie_js_get = df_graph[(df_graph['dst'] == node) & (df_graph['action'] == 'get_js')]

    cookie_js_set = df_graph[(df_graph['dst'] == node) & (df_graph['action'] == 'set_js')]

    cookie_get = df_graph[(df_graph['dst'] == node) & \
                    ((df_graph['action'] == 'get') | (df_graph['action'] == 'get_js'))]

    cookie_header = df_graph[(df_graph['dst'] == node) & \
                    (df_graph['action'] == 'get')]

    cookie_set = df_graph[(df_graph['dst'] == node) & \
                  ((df_graph['action'] == 'set') | (df_graph['action'] == 'set_js'))]

    localstorage_get = df_graph[(df_graph['dst'] == node) & \
                  (df_graph['action'] == 'get_storage_js')]

    localstorage_set = df_graph[(df_graph['dst'] == node) & \
    ...
    df_graph_gets = df_graph[(df_graph['action'] == 'get') | (df_graph['action'] == 'get_js') | (df_graph['action'] == 'get_storage_js')].copy()
    df_graph_sets = df_graph[(df_graph['action'] == 'set') | (df_graph['action'] == 'set_js') | (df_graph['action'] == 'set_storage_js')].copy()
    ...
    df_graph_gets_js = df_graph[(df_graph['action'] == 'get_js') | (df_graph['action'] == 'get_storage_js')].copy()
    df_graph_sets_js = df_graph[(df_graph['action'] == 'set_js') | (df_graph['action'] == 'set_storage_js')].copy()
    df_graph_gets_js['new_dst'] = df_graph_gets_js['dst'].apply(find_common_name)
    df_graph_sets_js['new_dst'] = df_graph_sets_js['dst'].apply(find_common_name)

    num_ls_gets_js = len(df_graph_gets_js[df_graph_gets_js['new_dst'] == node])
    num_ls_sets_js = len(df_graph_sets_js[df_graph_sets_js['new_dst'] == node])
...
def get_setter_features(df_graph, df_graph_indirect, node, setter_domain):
  ...
  try:
    setter_exfil = len(df_graph_indirect[df_graph_indirect['dst_domain'] == setter_domain])

    http_status = [300, 301, 302, 303, 307, 308]
    
    setter_redirects_sent = len(df_graph[(df_graph['src_domain'] == setter_domain) & (df_graph['response_status'].isin(http_status))])
    setter_redirects_rec = len(df_graph[(df_graph['dst_domain'] == setter_domain) & (df_graph['response_status'].isin(http_status))])

    setter_features = [setter_exfil, setter_redirects_sent, setter_redirects_rec]
    setter_feature_names = ['setter_exfil', 'setter_redirects_sent', 'setter_redirects_rec']
...
def get_indirect_features(G, df_graph, node):
  ...
  try:
    if len(df_graph) > 0:
      num_exfil = len(df_graph[(df_graph['src'] == node) & (df_graph['direction'] == 'out')])
      num_infil = len(df_graph[(df_graph['src'] == node) & (df_graph['direction'] == 'in')])
      num_infil_content = len(df_graph[(df_graph['src'] == node) & (df_graph['direction'] == 'in') & (df_graph['type'] == 'content')])
      num_url_exfil = len(df_graph[(df_graph['src'] == node) & (df_graph['direction'] == 'out') & (df_graph['type'] == 'url')])
      num_header_exfil = len(df_graph[(df_graph['src'] == node) & (df_graph['direction'] == 'out') & (df_graph['type'] == 'header')])
      num_body_exfil = len(df_graph[(df_graph['src'] == node) & (df_graph['direction'] == 'out') & (df_graph['type'] == 'postbody')])
      
      node_ls_name = node + "|$$|LS"
      ls_exfil = len(df_graph[(df_graph['src'] == node_ls_name) & (df_graph['direction'] == 'out')])
      ls_infil = len(df_graph[(df_graph['src'] == node_ls_name) & (df_graph['direction'] == 'in')])
      ls_infil_content = len(df_graph[(df_graph['src'] == node_ls_name) & (df_graph['direction'] == 'in') & (df_graph['type'] == 'content')])
      ls_url_exfil = len(df_graph[(df_graph['src'] == node_ls_name) & (df_graph['direction'] == 'out') & (df_graph['type'] == 'url')])
      ls_header_exfil = len(df_graph[(df_graph['src'] == node_ls_name) & (df_graph['direction'] == 'out') & (df_graph['type'] == 'header')])
      ls_body_exfil = len(df_graph[(df_graph['src'] == node_ls_name) & (df_graph['direction'] == 'out') & (df_graph['type'] == 'postbody')])
...
def get_dataflow_features(G, df_graph, node, G_indirect, G_indirect_all, df_indirect_graph):
  ...
  setter_domain = df_graph[(df_graph['graph_attr'] == 'Node') & (df_graph['name'] == node)]['setter_domain'].iloc[0]
  ...
  setter_features, setter_feature_names = get_setter_features(df_graph, df_indirect_graph, node, setter_domain)

  all_features = storage_features + rf_features + indirect_features + indirect_all_features + setter_features
...
def pre_extraction(G, df_graph, ldb):
  ...
  #G_indirect, df_indirect_graph = find_exfiltrations(G, df_graph)
  G_indirect = nx.DiGraph()

  df_exfiltration_edges = find_exfiltrations(G, df_graph, ldb)
  #df_local_edges = find_local_storage(G, df_graph)
  #df_indirect_graph = pd.concat([df_exfiltration_edges, df_local_edges]).reset_index()
  df_indirect_graph = df_exfiltration_edges.reset_index()

  if len(df_indirect_graph) > 0:
     G_indirect = nx.from_pandas_edgelist(df_indirect_graph, source='src', target='dst', edge_attr=True, create_using=nx.DiGraph())
