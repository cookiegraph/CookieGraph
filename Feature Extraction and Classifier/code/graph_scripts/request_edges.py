...
#     df_reqheaders = df_requests[['visit_id', 'request_id', 'url', 'headers', 'top_level_url', 'time_stamp']]
#     df_red = df_redirects[['visit_id', 'old_request_id', 'old_request_url', 'new_request_url', 'headers', 'response_status']]

#     x1 = pd.merge(df_red, df_reqheaders, left_on=['visit_id', 'old_request_id', 'old_request_url'], right_on=['visit_id', 'request_id', 'url'])
#     x2 = pd.merge(x1, df_requests, left_on=['visit_id', 'old_request_id', 'new_request_url'], right_on=['visit_id', 'request_id', 'url'])
#     x2 = x2.rename(columns={'headers_x': 'respattr1', 'headers_y': 'reqattr1', 'headers': 'reqattr2'})
#     x3 = pd.merge(x2, df_responses, left_on=['visit_id', 'old_request_id', 'new_request_url'], right_on=['visit_id', 'request_id', 'url'], how='outer')

#     df_redirect_edges = x3.groupby(['visit_id', 'old_request_id'], as_index=False).apply(process_redirects).reset_index()
#     df_redirect_edges = df_redirect_edges[[
#         'visit_id', 'src', 'dst', 'reqattr', 'respattr', 'response_status', 'time_stamp'
#     ]]
...
def get_cs_edges(df_requests, df_responses, call_stacks):
    ...
    #print(df_merge[df_merge.name==node])
    df_merge['cs_edges'] = df_merge.apply(process_call_stack, axis=1)
    #print(df_merge[df_merge.name==node])
    #print(df_merge[df_merge.index==0])
    #print(df_merge[df_merge.name==node])
